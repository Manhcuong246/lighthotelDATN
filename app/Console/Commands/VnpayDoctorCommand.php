...
class VnpayDoctorCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        $this->info('=== VNPay / môi trường online ===');
        $this->line('APP_URL:           '.$appUrl);
        $this->line('Return URL (vnp): '.$returnUrl);
        $this->line('VNPay gateway:     '.$vnpUrl);
        $this->line('TMN code:          '.($tmn !== '' ? $tmn : '(trống — kiểm tra .env)'));
        ...
        $issues = 0;
        if (! str_starts_with($appUrl, 'https://') && str_contains($vnpUrl, 'sandbox.vnpayment.vn')) {
