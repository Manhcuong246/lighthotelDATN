console.log('FIXED BOOKING FORM LOADING');

// Global functions - no class, no complexity
function renderRooms() {
    ...
    // Render guests immediately after HTML is set
    setTimeout(() => {
        for (let i = 0; i < roomCount; i++) {
            renderGuests(i);
        }
...
function renderGuests(roomIndex) {
    ...
    if (adults <= 0) {
        container.innerHTML = '<div class="text-muted">Vui lòng chọn số lượng người lớn</div>';
        return;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM LOADED - FIXED FORM');
...
console.log('FIXED BOOKING FORM READY');
