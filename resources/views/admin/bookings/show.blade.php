            ...
            <div class="card border-0 rounded-3 shadow-sm">
                <div class="card-body p-4">
                    ...
                    <div class="row mt-3 pt-3 border-top">
                        <div class="col-12">
                            <p class="text-uppercase small fw-bold text-muted mb-2">Thêm khách vào đơn</p>
                            <form action="{{ route('admin.bookings.store-guests', $booking) }}" method="POST" id="addGuestsForm">
                                @csrf
                                <div id="newGuestsContainer">
                                    <div class="row g-3 mb-3 guest-input-row">
                                        <div class="col-md-3">
                                            <label class="form-label small fw-bold">Họ tên</label>
                                            <input type="text" name="guests[0][name]" class="form-control" placeholder="Nhập họ tên" required>
                                        </div>
                                        <div class="col-md-2">
                                            <label class="form-label small fw-bold">Loại khách</label>
                                            <select name="guests[0][type]" class="form-select" required>
                                                <option value="adult">Người lớn</option>
                                                <option value="child_0_5">Trẻ 0-5 tuổi</option>
                                                <option value="child_6_11">Trẻ 6-11 tuổi</option>
                                            </select>
                                        </div>
                                        <div class="col-md-2">
                                            <label class="form-label small fw-bold">CCCD (nếu có)</label>
                                            <input type="text" name="guests[0][cccd]" class="form-control" placeholder="12 số" pattern="\d{12}" minlength="12" maxlength="12">
                                        </div>
                                        <div class="col-md-3">
                                            <label class="form-label small fw-bold">Phòng</label>
                                            <select name="guests[0][room_id]" class="form-select">
                                                <option value="">-- Chọn phòng --</option>
                                                @foreach($booking->bookingRooms as $br)
                                                    @if($br->room)
                                                        <option value="{{ $br->room->id }}">{{ $br->room->name }} ({{ $br->room->roomType->name ?? '' }})</option>
                                                    @endif
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-2 d-flex align-items-end">
                                            <button type="button" class="btn btn-danger btn-sm" onclick="this.closest('.guest-input-row').remove()">
                                ...
                                <div class="row">
                                    <div class="col-12">
                                        <button type="button" class="btn btn-outline-primary btn-sm me-2" onclick="addGuestRow()">
                                            <i class="bi bi-plus-lg"></i> Thêm khách
                                        </button>
                                        <button type="submit" class="btn btn-success btn-sm">
                                            <i class="bi bi-check-lg"></i> Lưu khách
                                        </button>
                                    </div>
                                </div>
                            </form>
                    ...
                    <script>
                        ...
                        function addGuestRow() {
                            const container = document.getElementById('newGuestsContainer');
                            const newRow = document.createElement('div');
                            newRow.className = 'row g-3 mb-3 guest-input-row';
                            newRow.innerHTML = `
                                <div class="col-md-3">
                                    <label class="form-label small fw-bold">Họ tên</label>
                                    <input type="text" name="guests[${guestRowIndex}][name]" class="form-control" placeholder="Nhập họ tên" required>
                                </div>
                                <div class="col-md-2">
                                    <label class="form-label small fw-bold">Loại khách</label>
                                    <select name="guests[${guestRowIndex}][type]" class="form-select" required>
                                        <option value="adult">Người lớn</option>
                                        <option value="child_0_5">Trẻ 0-5 tuổi</option>
                                        <option value="child_6_11">Trẻ 6-11 tuổi</option>
                                    </select>
                                ...
                                <div class="col-md-2">
                                    <label class="form-label small fw-bold">CCCD (nếu có)</label>
                                    <input type="text" name="guests[${guestRowIndex}][cccd]" class="form-control" placeholder="12 số" pattern="\d{12}" minlength="12" maxlength="12">
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label small fw-bold">Phòng</label>
                                    <select name="guests[${guestRowIndex}][room_id]" class="form-select">
                                        <option value="">-- Chọn phòng --</option>
                                        @foreach($booking->bookingRooms as $br)
                                            @if($br->room)
                                                <option value="{{ $br->room->id }}">{{ $br->room->name }} ({{ $br->room->roomType->name ?? '' }})</option>
